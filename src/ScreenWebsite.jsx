    ...
    <div className="page">
      ...
      {!data && !loading && (
        <div className="card" style={{ textAlign: 'center', padding: '48px 24px', maxWidth: 480, margin: '0 auto' }}>
          ...
          <h3 style={{ marginBottom: 8 }}>Connect Google Analytics</h3>
        ...
        return (
          <>
            <div className="grid-2" style={{ gap: 12 }}>
              ...
              <KpiCard label="Bounce rate" value={`${data.bounce_rate}%`}
                sub={data.bounce_rate < 60 ? '\u2713 Healthy' : '\u2191 Elevated'}
                color={data.bounce_rate < 60 ? 'var(--up)' : 'var(--dn)'}/>
              <KpiCard label="CVR" value={`${cvr}%`}
                sub={`${data.conversions?.toLocaleString()} conversions`}
                color={parseFloat(cvr) >= 1 ? 'var(--up)' : 'var(--dn)'}/>
            </div>

            {data.traffic_by_source?.length > 0 && (
              <div className="card">
            ...
            {data.traffic_by_source?.length > 0 && (
              <div className="card">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: '8px 16px', alignItems: 'center' }}>
                  ...
                  {data.traffic_by_source.map((row) => {
